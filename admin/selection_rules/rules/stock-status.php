...
class Pi_efrs_selection_rule_stock_status{
    public $slug;
    public $condition;
    ...
    function savedDropdown($html, $values, $count){
        ...
        $html .= Pi_efrs_selection_rule_main::createNumberField($count, $this->condition, $values,'any');
        $html .= '</span>';
        return $html;
    }

    function backOrderProductPresent($package, $package_support){
    ...
    function conditionCheck($result, $package, $logic, $values, $package_support){
                    ...
                    $or_result = false;
                    $back_order_present = $this->backOrderProductPresent($package, $package_support);
                    switch ($logic){
                        case 'all-in-stock':
...
new Pi_efrs_selection_rule_stock_status(PI_EFRS_SELECTION_RULE_SLUG);