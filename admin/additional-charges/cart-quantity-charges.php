...
class pisol_efrs_cart_quantity_extra_charges_setting{
    public $slug;
    ...
    function addTabContent($data){
       echo '<div class="p-2 border additional-charges-tab-content pi-active-tab" id="add-charges-tab-content-'.esc_attr($this->slug).'">';
    ...
    function rowTemplate($data){
        ...
        <tr>
        <td>Cart Quantity</td>
        <td class="pi-min-col"><input type="number" required name="pi_cart_quantity_charges[<?php echo esc_attr($count); ?>][min]" value="<?php echo esc_attr(self::value($row,'min')); ?>"  min="1" class="form-control"></td>
        <td class="pi-max-col"><input type="number" name="pi_cart_quantity_charges[<?php echo esc_attr($count); ?>][max]" value="<?php echo esc_attr(self::value($row,'max')); ?>"  min="1" class="form-control"></td>
        <td class="pi-fee-col"><input type="text" required name="pi_cart_quantity_charges[<?php echo esc_attr($count); ?>][charge]" value="<?php echo esc_attr(self::value($row,'charge')); ?>" class="form-control"></td>
    ...
    function formData($data){
        ...
        if ( isset( $action_value ) && 'edit' === $action_value ) {
            ...
            $data['pi_efrs_cart_quantity_sum_of_charges'] = get_post_meta($data['post_id'], 'pi_efrs_cart_quantity_sum_of_charges', true);

            $data['pi_efrs_cart_quantity_package_support'] = 'package';
        }else{
            $data['pi_enable_additional_charges_cart_quantity']               = '';
            $data['pi_cart_quantity_charges'] = '';

            $data['pi_efrs_cart_quantity_sum_of_charges'] = 'all';
    ...
    function cloneData($data, $post_id){
        ...
        $data['pi_efrs_cart_quantity_sum_of_charges'] = get_post_meta($post_id, 'pi_efrs_cart_quantity_sum_of_charges', true);

        $data['pi_efrs_cart_quantity_package_support'] = get_post_meta($post_id, 'pi_efrs_cart_quantity_package_support', true);
    ...
    function extraCharges($post_id, $package){
        ...
        $cart_quantity = self::getCartQty($post_id, $package);
    ...
    static function getCartQty($post_id, $package){
        ...
        if($package_support == 'cart'){
            ...
            $qty = PISOL\EFRS\Package::get_quantity($package);
        }
        
        return apply_filters('pi_efrs_extra_charge_cart_quantity',$qty);
    }
    ...
    static function getCharge($post_id, $get_charges, $cart_quantity){
        ...
        $summing_method =  get_post_meta($post_id, 'pi_efrs_cart_quantity_sum_of_charges', true);
...
new pisol_efrs_cart_quantity_extra_charges_setting();
